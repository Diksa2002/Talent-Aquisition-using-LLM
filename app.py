...
# =====================================================
# TEXT-TO-SPEECH (gTTS Human-like Voice Reader)
# =====================================================
...
def trigger_tts(text):
    """Fallback browser Web Speech API trigger."""
    if text:
...
with st.sidebar:
    ...
    current_step_idx = next((i for i, (s, _) in enumerate(steps) if s == st.session_state.step), 0)
...
# =====================================================
# STEP 1: UPLOAD RESUME & RESUME INTERVIEW TABS
# =====================================================

if st.session_state.step == "UPLOAD":
    st.markdown('<div class="main-title">Talent Acquisition Platform</div>', unsafe_allow_html=True)
    st.markdown('<div class="subtitle">Upload your resume to parse skills, match roles, and begin a dynamic interview, or restore an incomplete session.</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        with tab_resume:
            ...
            st.caption("If your interview was interrupted by a network issue or power outage, enter your registered email address below.")
            
            resume_email = st.text_input("Enter Registered Email Address", placeholder="e.g. candidate@example.com", key="input_resume_email")
            
            if st.button("Restore Active Interview Session", type="primary"):
                if not resume_email.strip():
                    st.warning("Please enter your registered email address.")
                else:
                    with st.spinner("Searching for active interview in MongoDB..."):
                        cand_doc = db.find_incomplete_candidate_by_email(resume_email)
                        if not cand_doc:
                            st.error("❌ No active, incomplete interview session found for this email address.")
                        else:
                            st.session_state.candidate_id = str(cand_doc["_id"])
                            st.session_state.selected_role = cand_doc.get("selected_role")
                            st.session_state.parsed_profile = {
                                "name": cand_doc.get("personal_info", {}).get("name", "Candidate"),
                                "email": cand_doc.get("personal_info", {}).get("email", ""),
                                "phone": cand_doc.get("personal_info", {}).get("phone", ""),
                                "current_location": cand_doc.get("personal_info", {}).get("current_location", ""),
                                "skills": cand_doc.get("extracted_skills", []),
                                "projects": cand_doc.get("projects", []),
                                "years_of_experience": cand_doc.get("years_of_experience", 0.0)
                            }
                            
                            existing_qas = cand_doc.get("qas", [])
                            st.session_state.interview_qas = existing_qas
                            
                            if existing_qas:
                                last_qa = existing_qas[-1]
                                if last_qa.get("answer"):
                                    role_data = db.get_role_by_name(cand_doc.get("selected_role"))
                                    role_desc = role_data.get("job_description", "") if role_data else ""
                                    next_q = interview.generate_next_question(
                                        cand_doc.get("selected_role"),
                                        role_desc,
                            ...
                            st.success("Session restored successfully! Resuming interview...")
                            st.rerun()
...
# =====================================================
    ...
    # Speak the current question using gTTS human-like voice
    ...
    if not last_qa.get("answer"):
        ...
        with col_rec:
            # Use mic_recorder for candidate voice input
            audio = mic_recorder(
        ...
        st.info("Interview conversation completed.")

# =====================================================
# STEP 4: THANK YOU REPORT
# =====================================================
